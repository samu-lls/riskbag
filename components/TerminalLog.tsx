...
const getLogStyle = (log: string) => {
  if (log.includes('💥') || log.includes('DANO') || log.includes('ELIMINADO') || log.includes('💀') || log.includes('expulso'))
  ...
  if (log.includes('Mercado Negro') || log.includes('rodada') || log.includes('partida') || log.includes('Rodada'))
...
export default function TerminalLog({ logs }: { logs: string[] }) {
  ...
  useEffect(() => {
    // Scroll only the internal div — never touches window scroll position
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ fontFamily: 'var(--font-mono)', overscrollBehavior: 'contain' }}
      >
        {logs.length === 0 ? (
          <span className="text-[11px] italic mt-2" style={{ color: 'rgba(200,208,218,0.2)' }}>
            _ aguardando eventos...
            <span style={{ animation: 'terminalBlink 1s step-end infinite', display: 'inline-block', marginLeft: 2 }}>█</span>
          </span>
        ) : (
          logs.map((log, idx) => {
            ...
            return (
              <motion.div
                key={idx}
                initial={isRecent ? { opacity: 0, x: -6 } : false}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.15 }}
                className="terminal-line"
                style={{ color, textShadow: glow ? `0 0 8px ${glow}` : 'none', fontSize: 11 }}
              >
                {log}
              </motion.div>
            );
